...
また **「直近の再描画」** に、ズーム/パンの再描画にかかった時間 (ms) を表示する。
MapLibre の `movestart`（再描画開始）から `idle`（カメラ停止 + 必要タイル全ロード +
描画完了）までを `performance.now()` で計測したもの。レンダリング自体は MapLibre が行うため、
この時間はタイル取得（通信）が支配的で、OPFS キャッシュの ON/OFF で差が分かる
（`idle` は記号のフェード完了も含むため一定のオーバーヘッドが乗る点に注意）。
