...
- **以降は 2 指標**:
  - `movestart → idle`: 操作全体（**スワイプで指を置いている時間も含む**）。
  - `moveend → idle`（「うち 移動停止後の描画」）: カメラの移動が止まってから描画完了まで＝**実際のレンダリング時間**。スマホで長くスワイプしても、この値は描画分だけになるので「操作が長い」だけなのに遅く見える誤解を防ぐ。`move`/`moveend` で「最後にカメラが動いた時刻」を追って `idle` との差を取る（イベント順序に依存しない）。

パネルは**タイトルをタップで折りたたみ**できる（スマホの小さい画面で地図を広く使うため）。
